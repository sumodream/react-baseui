  ...
  _mountImageIntoNode: function (markup, container, instance, shouldReuseMarkup, transaction) {
    ...
    if (shouldReuseMarkup) {
      ...
      if (ReactMarkupChecksum.canReuseMarkup(markup, rootElement)) {
        ...
        var difference = ' (client) ' + normalizedMarkup.substring(diffIndex - 100, diffIndex + 400) + '\n (server) ' + rootMarkup.substring(diffIndex - 100, diffIndex + 400);
...
function dangerousStyleValue(name, value, component) {
  ...
  if (isEmpty) {
    return '';
        ...
        //small fix by wesley 2016-05-27,padding 4value set without px will fail,we don't want that
      if (['padding','margin'].indexOf(name)>=0){
        var values = (""+value).split(" ");
        if (values.length>1){
        var value =  values.map((v)=>{
          if ((/^\d+$/.test(v)) && (v!='0')){
				  	return v+"px";
				  }
          else{
