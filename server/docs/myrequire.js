...
function renderPage(){
    ...
    if (!pageCode){
        // require('superagent').get("/server/main.js").end(function(err,rsp){
         //    pageCode = babel(rsp.text).code;
         //    eval(pageCode);
		// });
        require('superagent').get("/server/init.js").end(function(err,rsp){
           pageCode = babel(rsp.text).code;
           eval(pageCode);
        });
    }
...
function require(param){
    ...
    if (param == "react-markdown") return reactMarkdown;
    ...
    if (param == "draft-js") return Draft;
    if (param == "redux") return Redux;
    if (param == "remarkable") return Remarkable;
